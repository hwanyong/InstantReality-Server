...
from ..widgets import TopDownWidget, SideElevation3LinkWidget, SideElevation4LinkWidget, WristPitchGaugeWidget, TopDownWristWidget


class QuadViewTab(BaseTabController):
    """
    Tab 4: Slot 1+2+3+4 with Full Arm IK.
    - Top View: X/Y sliders, θ1 auto-calculated (same as TripleViewTab)
    - Side View: Z slider, θ2/θ3 IK-calculated, 3-Link FK rendering
    """
    
    # Range constants (same as TripleViewTab)
    X_MIN = -300
    X_MAX = 300
    Y_MIN = -300
    Y_MAX = 300
    ...
    def build_ui(self):
        ...
        self.theta4_var = tk.DoubleVar(value=0.0)  # θ4 for Slot 4 (Wrist Pitch)
        ...
        self._create_slot4_view(self.main_frame)
    
    def _create_top_view(self, parent):
        """Create Top-Down view for X/Y input (same as TripleViewTab)."""
    ...
    def _create_slot4_view(self, parent):
        """Create Slot 4 (Wrist Pitch) control panel with canvases."""
        frame = ttk.LabelFrame(parent, text="Slot 4 (Wrist Pitch)", padding=5)
        frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=2)
        
        # Canvas Row
        canvas_row = ttk.Frame(frame)
        canvas_row.pack(fill=tk.X, pady=2)
        
        # 4-Link Side View Canvas
        side4_frame = ttk.Labelframe(canvas_row, text="Side (R-Z)", padding=2)
        side4_frame.pack(side=tk.LEFT, padx=2)
        self.slot4_side_canvas = tk.Canvas(side4_frame, width=140, height=140, bg="#2e1a2e")
        self.slot4_side_canvas.pack()
        self.slot4_side_widget = SideElevation4LinkWidget(self.slot4_side_canvas, {'canvas_size': 140})
        
        # Top-Down View Canvas
        topdown_frame = ttk.Labelframe(canvas_row, text="Top (X-Y)", padding=2)
        topdown_frame.pack(side=tk.LEFT, padx=2)
        self.slot4_top_canvas = tk.Canvas(topdown_frame, width=120, height=120, bg="#1a2e1a")
        self.slot4_top_canvas.pack()
        self.slot4_top_widget = TopDownWristWidget(self.slot4_top_canvas, {'canvas_size': 120})
        
        # θ4 Gauge Canvas
        gauge_frame = ttk.Labelframe(canvas_row, text="θ4 Gauge", padding=2)
        gauge_frame.pack(side=tk.LEFT, padx=2)
        self.slot4_gauge_canvas = tk.Canvas(gauge_frame, width=100, height=100, bg="#1a1a2e")
        self.slot4_gauge_canvas.pack()
        self.slot4_gauge_widget = WristPitchGaugeWidget(self.slot4_gauge_canvas, {'canvas_size': 100})
        
        # θ4 Slider
        slider_frame = ttk.Frame(frame)
        slider_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(slider_frame, text="θ4:", font=("Consolas", 10)).pack(side=tk.LEFT)
        self.theta4_slider = ttk.Scale(
            slider_frame, from_=-90, to=90,
            variable=self.theta4_var, orient=tk.HORIZONTAL, length=160,
            command=lambda v: self.update_visualization()
        )
        self.theta4_slider.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Real-time State Frame
        dyn_frame_s4 = ttk.Labelframe(frame, text="Slot 4 State", padding=5)
        dyn_frame_s4.pack(fill=tk.X, pady=(5, 0))
        
        self.lbl_theta4 = ttk.Label(dyn_frame_s4, text="θ4: 0.0°", font=("Consolas", 9))
        self.lbl_theta4.pack(anchor="w")
        
        self.lbl_pulse4 = ttk.Label(dyn_frame_s4, text="Pulse: --", font=("Consolas", 9))
        self.lbl_pulse4.pack(anchor="w")
        
        self.lbl_valid4 = ttk.Label(dyn_frame_s4, text="", font=("Consolas", 9))
        self.lbl_valid4.pack(anchor="w")
        
        # Slot 4 Config
        s4_frame = ttk.Labelframe(frame, text="Config", padding=3)
        s4_frame.pack(fill=tk.X, pady=(5, 0))
        
        self.lbl_config_s4 = ttk.Label(s4_frame, text="--", font=("Consolas", 7), justify=tk.LEFT)
    ...
    def on_enter(self):
        ...
        self.slot4_side_widget.draw_static()
        self.slot4_top_widget.draw_static()
        self.slot4_gauge_widget.draw_static()
    ...
    def update_visualization(self):
        ...
        # θ1 Calculation (same as TripleViewTab)
        ...
        # --- Slot 4 Real-time State ---
        theta4 = self.theta4_var.get()
        pulse_calc_s4 = "--"
        phy_angle_s4 = theta4
        valid4 = False
        ...
        if hasattr(self, 'p4'):
            ...
            polarity4 = self.p4.get('polarity', 1)
            
            # horizontal, min_pos: left -> polarity = -1
            phy_angle_s4 = zero_offset4 + (theta4 * polarity4)
        ...
        self.lbl_theta4.config(text=f"θ4: {theta4:.1f}° (Math) | {phy_angle_s4:.1f}° (Phy)")
        self.lbl_pulse4.config(text=f"Pulse: {pulse_calc_s4}")
        self.lbl_valid4.config(
            text="✓ Valid" if valid4 else "✗ Out of Range",
            foreground="#44ff44" if valid4 else "#ff4444"
        )
        
        # --- Update Slot 4 Widgets ---
        self.slot4_side_widget.update_target(theta2, theta3, theta4)
        self.slot4_gauge_widget.update_target(theta4, valid4)
        
        # Top-Down Wrist View: θ1, R (horizontal distance), a4, θ4
        a4 = getattr(self, 'p4', {}).get('length', 65.0) if hasattr(self, 'p4') else 65.0
        self.slot4_top_widget.update_target(theta1, R, a4, theta4)
    ...
    def send_command(self, duration):
        """Send command - All 4 slots using IK-calculated angles + manual θ4."""
        if not all(hasattr(self, k) for k in ['p1', 'p2', 'p3', 'p4']):
            self.log("[QuadView] Config not fully loaded, cannot send.")
            return
        ...
        # Slot 4 (horizontal, min_pos: left → polarity = -1)
        zero_off4 = self.p4.get('zero_offset', 0)
        polarity4 = self.p4.get('polarity', 1)
        phy4 = zero_off4 + (theta4 * polarity4)
        ...
        self.log(f"[QuadView] Sent Ch{self.p1['channel']}:{pls1} (θ1={theta1:.1f}°)")
        self.log(f"[QuadView] Sent Ch{self.p2['channel']}:{pls2} (θ2={theta2:.1f}° IK, Phy={phy2:.1f}°)")
        self.log(f"[QuadView] Sent Ch{self.p3['channel']}:{pls3} (θ3={theta3:.1f}° IK, Phy={phy3:.1f}°)")
        self.log(f"[QuadView] Sent Ch{self.p4['channel']}:{pls4} (θ4={theta4:.1f}°, Phy={phy4:.1f}°)")
